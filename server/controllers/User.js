...
import Workout from "../models/Workout.js"; // Adjust path if necessary
...
export const UserRegister = async (req, res, next) => {
  try {
    ...
    if (!email || !password || !name) {
      return next(createError(400, "All fields are required (name, email, password)."));
    }
...
export const UserLogin = async (req, res, next) => {
  try {
    ...
    // Debug: Log user and password details for debugging
    ...
    // Debug: Check password comparison result
    ...
    const token = jwt.sign({ id: user._id }, process.env.JWT, {
      expiresIn: "9999 years", // Consider reducing this for better security
    });
